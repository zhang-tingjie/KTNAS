...
class Net(nn.Module):
    def __init__(self, n_blocks, n_channels, n_classes, dropout, ss):
        ...
        self.conv2 = nn.Conv2d(n_channels, 2 * n_channels, 1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(2 * n_channels)

        self.conv2_prev = nn.Conv2d(n_channels, 2 * n_channels, 1, stride=1, padding=0, bias=False)
        ...
        self.conv3 = nn.Conv2d(2 * n_channels, 4 * n_channels, 1, stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(4 * n_channels)

        self.conv3_prev = nn.Conv2d(2 * n_channels, 4 * n_channels, 1, stride=1, padding=0, bias=False)
        ...
        self.fc1 = nn.Sequential(nn.ReLU(), nn.Linear(4 * n_channels, n_classes))
    ...
    def forward(self, x):
        ...
        x, x_prev = self.block_1(x_prev, x_prev) # x_prev:s_0 and s_1
