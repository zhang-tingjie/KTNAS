...
def get_dataset(config, dataset_name, is_test=False, is_all=False):
    ...
    if is_test:
        if dataset_name == 'PathMNIST':
            ...
            return get_mnist_test(config, os.path.join(data_path, 'MNIST'))
        elif dataset_name == 'FASHION':
            return get_mnist_test(config, os.path.join(data_path, 'FASHION'), dataset_name='FASHION')
        ...
        if dataset_name == 'CIFAR10':
            ...
            return get_mnist(config, os.path.join(data_path, 'MNIST'))
        elif dataset_name == 'FASHION':
            return get_mnist(config, os.path.join(data_path, 'FASHION'), dataset_name='FASHION')
...
def get_mnist(config, data_path, dataset_name='MNIST'):
    train_transform = transforms.Compose([])
    normalize = transforms.Normalize((0.1307,), (0.3081,))
    train_transform.transforms.append(transforms.ToTensor())
    train_transform.transforms.append(transforms.RandomCrop(28, padding=2))
    train_transform.transforms.append(transforms.RandomHorizontalFlip())
    train_transform.transforms.append(transforms.Lambda(lambda x: x.repeat(3, 1, 1)))
    train_transform.transforms.append(normalize)
    ...
    if dataset_name == 'MNIST':
        ...
        trainset_train, trainset_val = torch.utils.data.random_split(trainset, [train_size, val_size])
        ...
        trainset = torchvision.datasets.FashionMNIST(root=data_path, train=True, download=True, transform=train_transform)
...
def get_mnist_test(config, data_path, dataset_name='MNIST'):
    train_transform = transforms.Compose([])
    normalize = transforms.Normalize((0.1307,), (0.3081,))
    train_transform.transforms.append(transforms.ToTensor())
    train_transform.transforms.append(transforms.RandomCrop(28, padding=2))
    ...
    if dataset_name == 'MNIST':
        ...
        test_data = torchvision.datasets.MNIST(root=data_path, train=False, download=True, transform=train_transform)
        test_loader = torch.utils.data.DataLoader(test_data, batch_size=config.get('batch_size_val'), shuffle=True, num_workers=8)

    elif dataset_name == 'FASHION':
        n_class = 10
        
        test_data = torchvision.datasets.FashionMNIST(root=data_path, train=False, download=True, transform=train_transform)
        test_loader = torch.utils.data.DataLoader(test_data, batch_size=config.get('batch_size_val'), shuffle=True, num_workers=8)
