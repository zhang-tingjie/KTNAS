...
class Conv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding):
        super(DilConv, self).__init__()
        conv = nn.Conv2d(in_channels, out_channels, kernel_size, padding, bias=False),

    def forward(self, x):
        return self.conv(x)


def generate_conv(in_channels, out_channels, kernel):
    padding = int((kernel - 1) / 2)
    conv = Conv(in_channels, out_channels, kernel, padding)
    return conv


def conv1x1(in_channels, out_channels):
    return generate_conv(in_channels, out_channels, 1)


def conv3x3(in_channels, out_channels):
    return generate_conv(in_channels, out_channels, 3)
...
def zero(in_channels, out_channels):
    return Zero()
