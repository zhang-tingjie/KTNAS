...
def main():
    ...
    parser.add_argument('--Task_1', type=str, choices=['MNIST','FASHION'], help='the working data', default='MNIST')
    parser.add_argument('--Task_2', type=str, choices=['MNIST','FASHION'], help='the working data', default='FASHION')
    ...
    ga = gnas.genetic_algorithm_searcher(ss, generation_size=config.get('generation_size'),
                                         ...
                                         n_transferred_individual=config.get('n_transferred_individual'),
                                         n_generation_saved_negative=config.get('n_generation_saved_negative'),
                                         ratio_elite_individual=config.get('ratio_elite_individual'))
    ######################################
    # Loss function
    ######################################
    ...
    for generation in range(config.get('n_generations')):  # loop over the dataset multiple times
        ...
        if generation == 0:
            ...
            # initialize transfer population
            ...
            # save parent individuals
            ga.save_old_current_generation() 
            
            _, _, _, _,  n_diff_1, _, _, _, _,  n_diff_2 = ga.first_population()
            ...
            # Select top K individuals from P & O & TP
